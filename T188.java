 ...
 * normal O(n^2*k) solution can't pass the OJ 209/211
 * modify k > n/2, can't pass 211/211
 * using sell/buy mixed array to update - O(n*k)
 */
public class T188 {
    public int maxProfit(int k, int[] prices){
        ...
        Arrays.fill(revenue,Integer.MIN_VALUE);
        for(int i=0;i<prices.length;i++)
            for(int j=0;j<k;j++){
                revenue[j] = Math.max(revenue[j],(j==0?0:profit[j-1])-prices[i]);
                profit[j] = Math.max(profit[j],prices[i]+revenue[j]);
            }
        return profit[k-1];
    }
    ...
    public int maxProfitG(int k, int[] prices){
        ...
        for(int i=1;i<k+1;i++){
            ...
            for(int j=1;j<len;j++){
                ...
                dp[i][j] = Math.max(dp[i][j],tmpMax);
            }
        }
        return dp[k][len-1];
    }
//    private int maxProfitR(int[][] dp, int[] prices, int k, int n){
//        if(k==0 || n==0) return 0;
//        if(dp[k][n]>0) return dp[k][n];
//        dp[k][n] = maxProfitR(dp,prices,k,n-1);
//        for(int i=0;i<n;i++){
//            int tmpMax = maxProfitR(dp,prices,k-1,i)+prices[n]-prices[i];
//            if(tmpMax>dp[k][n]) dp[k][n] = tmpMax;
//        }
//        return dp[k][n];
//    }
//    public int maxProfitR(int k, int[] prices){
//        if(k==0 || prices.length<2) return 0;
//        if(k>=prices.length/2) return maxProfit(prices);
//        int[][] dp = new int[k+1][prices.length];
//        return maxProfitR(dp,prices,k,prices.length-1);
//    }
    ...
    public static void main(String[] args) {
        ...
        t.maxProfit(100,prices);
        long s1 = System.nanoTime();
        System.out.println((s1-s0)/1e6);
    }
}
